...
fn main() {
    ...
    let doc_file_path = std::env::var("OUT_DIR").unwrap() + "/rhai-net-docs.md";
...
mod doc_gen {
    ...
    // Rhai modules in the `rhai-net` package.
    mod pkg {
        include!("src/addr.rs");
        include!("src/tcp.rs");
    }
    ...
    pub fn generate_doc(writer: &mut impl Write) {
        ...
        let mut net_module = Module::new();
        combine_with_exported_module!(&mut net_module, "rhai_net_addr", pkg::addr_functions);
        combine_with_exported_module!(&mut net_module, "rhai_net_tcp", pkg::tcp_functions);
        engine.register_global_module(net_module.into());
...
// Bad hack to get build.rs to have `convert_to_int`.
#[cfg(feature = "metadata")]
pub(crate) mod util {
